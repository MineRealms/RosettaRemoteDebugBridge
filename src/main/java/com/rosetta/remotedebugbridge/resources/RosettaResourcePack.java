 ...
 */
package com.rosetta.remotedebugbridge.resources;
...
public class RosettaResourcePack
implements PackResources {
    private final Path rootPath;
    private final PackType packType;
    private final Set<String> namespaces;

    public RosettaResourcePack(Path rootPath, PackType packType) {
    ...
    private void scanNamespaces() {
        ...
        try (Stream<Path> paths = Files.list(this.rootPath);){
            ...
            RosettaRemoteDebugBridge.LOGGER.error("Failed to scan namespaces in: {}", (Object)this.rootPath, (Object)e);
        }
        if (!this.namespaces.isEmpty()) {
            RosettaRemoteDebugBridge.LOGGER.info("Found namespaces in RosettaRemoteDebugBridge {}: {}", (Object)(this.packType == PackType.CLIENT_RESOURCES ? "assets" : "data"), this.namespaces);
        }
    }
    ...
    public IoSupplier<InputStream> getRootResource(String ... paths) {
        ...
        if ("pack.mcmeta".equals(fileName)) {
            String meta = "{\n    \"pack\": {\n        \"pack_format\": 15,\n        \"description\": \"RosettaRemoteDebugBridge Dynamic Resources\"\n    }\n}\n";
    ...
    public IoSupplier<InputStream> getResource(PackType type, ResourceLocation location) {
        ...
        if (Files.exists(file, new LinkOption[0]) && Files.isRegularFile(file, new LinkOption[0])) {
            RosettaRemoteDebugBridge.LOGGER.debug("Loading resource: {}", (Object)file);
            return () -> Files.newInputStream(file, new OpenOption[0]);
        }
        return null;
    }
        ...
        try (Stream<Path> paths = Files.walk(namespacePath, new FileVisitOption[0]);){
            paths.filter(x$0 -> Files.isRegularFile(x$0, new LinkOption[0])).forEach(file -> {
                try {
                    ...
                    RosettaRemoteDebugBridge.LOGGER.error("Error visiting resource: {}", file, (Object)e);
                }
            });
        }
        catch (IOException e) {
            RosettaRemoteDebugBridge.LOGGER.error("Failed to list resources in: {}", (Object)namespacePath, (Object)e);
        }
    }
    ...
    public <T> T getMetadataSection(MetadataSectionSerializer<T> deserializer) throws IOException {
        try {
            if (deserializer.getMetadataSectionName().equals("pack")) {
                ...
                pack.addProperty("description", "RosettaRemoteDebugBridge Dynamic Resources");
                return (T)deserializer.fromJson(pack);
            }
        }
        catch (Exception e) {
            RosettaRemoteDebugBridge.LOGGER.error("Error reading pack metadata", (Throwable)e);
        }
        return null;
    }

    public String packId() {
        return "rosetta_remote_debug_bridge_" + (this.packType == PackType.CLIENT_RESOURCES ? "assets" : "data");
    }
