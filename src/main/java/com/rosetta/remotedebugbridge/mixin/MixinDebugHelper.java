...
public class MixinDebugHelper {
    public static boolean validateMixinSetup(Path jarFile, Path configFile, Logger logger) {
        ...
        try {
            ...
            try (JarFile jar = new JarFile(jarFile.toFile());){
                JarEntry configEntry = jar.getJarEntry("rosetta.mixins.json");
                if (configEntry == null) {
                    logger.error("\u2717 CRITICAL: rosetta.mixins.json NOT in JAR root!");
                    hasErrors = true;
                } else {
    ...
    public static void showFixSuggestions(Path classesDir, Path configFile, Logger logger) {
        ...
        try {
            ...
            logger.info("3. Example: if package is 'rosetta.startup.mixins', file should be at:");
            logger.info("   RosettaRemoteDebugBridge/startup/mixins/ItemStackMixin.java");
