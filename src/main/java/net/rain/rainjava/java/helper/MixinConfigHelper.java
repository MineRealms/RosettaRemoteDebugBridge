...
public class MixinConfigHelper {
    public static void initializePlugin(MixinConfig config) {
        try {
            ...
            if (plugin != null) {
                ...
                RainJava.LOGGER.info("\u2705 Successfully set plugin for MixinConfig: {}", (Object)config.getName());
            } else {
                RainJava.LOGGER.warn("\u26a0\ufe0f Could not create or find a plugin for MixinConfig");
            }
        }
        catch (Exception e) {
            RainJava.LOGGER.error("Failed to initialize plugin for MixinConfig", (Throwable)e);
    ...
    private static PluginHandle getPluginFromExistingConfig() {
        try {
            ...
            if (!configs.isEmpty() && (pluginField = MixinConfigHelper.findPluginField(firstConfig = (MixinConfig)configs.get(0))) != null) {
                ...
                if (plugin != null) {
                    RainJava.LOGGER.info("Found existing plugin from config: {}", (Object)firstConfig.getName());
                    return plugin;
                }
            }
        }
        catch (Exception e) {
            RainJava.LOGGER.debug("Could not get plugin from existing config", (Throwable)e);
        }
        return null;
    }
    ...
    private static PluginHandle createDefaultPlugin(MixinConfig config) {
        try {
            ...
            RainJava.LOGGER.error("Failed to create default PluginHandle\uff0c\u5c1d\u8bd5\u5907\u7528\u624b\u6bb5\uff01", (Throwable)e);
            try {
                Class<?> serviceClass = Class.forName("org.spongepowered.rain.asm.service.MixinService");
                Object service = serviceClass.getMethod("getService", new Class[0]).invoke(null, new Object[0]);
                if (service == null || !(service instanceof IMixinService)) {
                ...
                RainJava.LOGGER.error("Failed to create default PluginHandle", (Throwable)err);
                return null;
            }
        }
    }
